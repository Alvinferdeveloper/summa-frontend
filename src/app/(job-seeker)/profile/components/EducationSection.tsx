...
export default function EducationSection({ educations }: EducationSectionProps) {
  ...
  return (
    <Card className="border-2 border-border/60 shadow-md hover:shadow-lg transition-all duration-300 rounded-xl overflow-hidden">
      <CardHeader className="flex flex-row items-center justify-between pb-4 border-b border-border/50 bg-gradient-to-r from-primary/5 via-primary/3 to-transparent">
      ...
      <CardContent className="pt-6">
        {educations && educations.length > 0 ? (
          <div className="space-y-6">
            {educations.map((edu, index) => (
              <div
                key={edu.id}
                className={`relative pl-8 pb-6 ${index !== educations.length - 1
                  ? "border-l-2 border-gradient-to-b from-primary/40 via-primary/20 to-transparent"
                  : ""
                  }`}
              >
                <div className="absolute left-0 top-2 -translate-x-1/2 w-4 h-4 rounded-full bg-gradient-to-br from-primary to-primary/70 border-2 border-background shadow-md transition-transform duration-200 hover:scale-125" />
                ...
                <div className="group hover:bg-primary/5 rounded-xl p-6 border border-border/60 hover:border-primary/50 hover:shadow-lg transition-all duration-300">
                  <div className="flex flex-col sm:flex-row gap-5">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="mb-4">
                        <h3 className="text-lg font-bold text-foreground mb-1.5 text-balance leading-tight">
                          {edu.degree}
                        </h3>
                        {edu.field_of_study && (
                          <p className="text-sm font-medium text-primary/80 mb-2 capitalize">{edu.field_of_study}</p>
                        )}
          ...
          <div className="text-center py-12 px-4">
            ...
            <Link href="/profile/edit#education">
              <Button
                variant="outline"
                className="gap-2 bg-transparent text-primary hover:bg-primary/10 border-primary/30 hover:border-primary"
              >
        ...
        {educations && educations.length > 0 && (
          <Link
            ...
            className="inline-flex mx-auto items-center gap-2 text-sm font-semibold text-primary hover:text-primary/80 transition-colors mt-6 group"
          >
