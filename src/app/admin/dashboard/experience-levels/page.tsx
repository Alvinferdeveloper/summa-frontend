...
import ExperienceLevelForm from "./components/ExperienceLevelForm";
import DeleteConfirmationModal from "../components/DeleteConfirmationModal";
import {
  useAdminExperienceLevels,
  useCreateExperienceLevel,
  useUpdateExperienceLevel,
  useDeleteExperienceLevel,
  type ExperienceLevel,
} from "./hooks/useExperienceLevels";
import { Card, CardContent, CardDescription, CardHeader, CardTitle } from "@/components/ui/card";

export default function ExperienceLevelsPage() {
  const { data: experienceLevels, isLoading } = useAdminExperienceLevels();
  ...
  const [selectedItem, setSelectedItem] = useState<ExperienceLevel | null>(null);
  const [isDeleteModalOpen, setIsDeleteModalOpen] = useState(false);
  const [itemToDelete, setItemToDelete] = useState<ExperienceLevel | null>(null);
  ...
  const handleEdit = (item: ExperienceLevel) => {
    setSelectedItem(item);
    setIsModalOpen(true);
  };

  const handleDeleteClick = (item: ExperienceLevel) => {
  ...
  const columns: ColumnDef<ExperienceLevel>[] = [
    {
      accessorKey: "name",
      header: ({ column }) => (
      ...
      <Card className="shadow-md border-border/50 rounded-md">
        <CardHeader>
          <div className="flex justify-between items-center">
            <div>
              <CardTitle className="text-3xl font-bold">Niveles de Experiencia</CardTitle>
              <CardDescription>Gestiona los niveles de experiencia de empleo del sistema.</CardDescription>
            </div>
            <Dialog open={isModalOpen} onOpenChange={setIsModalOpen}>
              ...
              <DialogContent>
                <DialogHeader>
                  <DialogTitle>{selectedItem ? 'Editar' : 'Crear'} Nivel de Experiencia</DialogTitle>
                </DialogHeader>
                <ExperienceLevelForm
        ...
        <CardContent>
          <DataTable columns={columns} data={experienceLevels || []} />
        </CardContent>
      </Card>
    </div>
  );
