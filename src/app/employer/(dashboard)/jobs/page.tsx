...
export default function MyJobsPage() {
  ...
  const [selectedApplicationIdForInterview, setSelectedApplicationIdForInterview] = useState<number | null>(null);
  const [selectedApplicantNameForInterview, setSelectedApplicantNameForInterview] = useState<string>('');
  const [selectedJobTitleForInterview, setSelectedJobTitleForInterview] = useState<string>('');
  ...
  const handleScheduleInterview = (jobApplicationId: number, applicantName: string, jobTitle: string) => {
    setSelectedApplicationIdForInterview(jobApplicationId);
    setSelectedApplicantNameForInterview(applicantName);
  ...
  return (
    <>
      <ScheduleInterviewModal
        ...
        jobApplicationId={selectedApplicationIdForInterview!}
        applicantName={selectedApplicantNameForInterview}
        jobTitle={selectedJobTitleForInterview}
      />
                ...
                {isLoadingApplicants ? (
                  ...
                  <div className="p-4 space-y-3">
                    {applicants.map((app) => (
                      <div
                        key={app.id}
                        className="group relative flex flex-col gap-4 p-5 border border-slate-200 rounded-xl hover:border-blue-300 hover:shadow-md bg-white transition-all duration-200"
                      >
                                ...
                                {app.status === "Postulado" && (
                                  <DropdownMenuItem
                                    onClick={(e) => {
                                      e.preventDefault();
                                      e.stopPropagation();
                                      handleScheduleInterview(app.id, `${app.applicant.first_name} ${app.applicant.last_name}`, selectedJob?.title || '');
                                    }}
                        ...
                        {app.interview && app.interview.candidate_response_status !== 'Pendiente' && (
                          <div className="w-full mt-3 pt-3 border-t border-slate-200">
                            <h4 className="text-sm font-semibold mb-2">Respuesta a la Entrevista:</h4>
                            <div className="flex items-center gap-4">
                              {app.interview.candidate_response_status === 'Aceptada' ? (
                                <Badge variant="default" className="bg-green-500 hover:bg-green-600">
                                  <CheckCircle className="mr-2 h-4 w-4" /> Aceptada
                                </Badge>
                              ) : (
                              ...
                              {app.interview.requested_accommodations && (
                                <TooltipProvider>
                                  <Tooltip>
                                    ...
                                    <TooltipContent>
                                      <p className="max-w-xs">{app.interview.requested_accommodations}</p>
                                    </TooltipContent>
                                  </Tooltip>
                                </TooltipProvider>
                              )}
